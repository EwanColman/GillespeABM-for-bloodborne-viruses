...
import pandas as pd
import random
import network_generator as net
import pickle as pk
...
def plot_output(disease):
    ...
    plt.legend(loc=1,ncol=2,prop={'size':fs})
    
    
    plt.xlim([0,end_time*time_steps_per_year])
    #plt.ylim([0,100])
    plt.ylim([0,max([max(population[i]) for i in population])])
    #plt.xticks([])
    #plt.yticks([])
    ...
    #plt.savefig('../Meetings/2024-06-17_whole_team/figs/one_run'+disease+'.png',format='png',dpi=300,bbox_inches='tight')
...
def create_node():
    ...
    '''
    trans_state=''
    if random.random()<HIV_prevalence:
        trans_state=trans_state+'A'
    else:
...
for run in range(100):
    ...
    output['HIV'].append(HIV_population)
    output['HCV'].append(HCV_population)

    print(run,timer()-start)

#pk.dump(output,open('pickles/model_outputs_random_interventions.p','wb'))
#pk.dump(output,open('pickles/model_outputs_targeted_interventions.p','wb'))
pk.dump(output,open('pickles/model_outputs_no_interventions.p','wb'))
